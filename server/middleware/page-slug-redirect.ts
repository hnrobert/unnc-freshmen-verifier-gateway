 ...
 * even after a new page reclaims the slug.
 */
export default defineEventHandler(async (event) => {
  ...
  const page = await AppDataSource.getRepository(Page).findOneBy({ id: row.pageId })
  // Stale entry: the page is gone, or now owns this slug directly (renamed back).
  // Drop the row and let normal routing handle the request.
  if (!page || page.slug === seg) {
    await redirectRepo.delete({ oldSlug: seg })
    return
  }

  const rest = parts.slice(2).join('/')
  const target = '/' + page.slug + (rest ? '/' + rest : '') + search
