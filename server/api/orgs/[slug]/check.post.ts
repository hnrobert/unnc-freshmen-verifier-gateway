...
// Public admission check for an org. Mirrors the old /api/check contract:
// accepts {username|name, userid|id_number|idNumber}, always 200 with an
// AdmissionResult on a completed query (not-found / captcha failure included).
export default defineEventHandler(async (event) => {
  const slug = getRouterParam(event, 'slug') as string
  const config = loadOrgConfig(slug) // throws 404 if org missing
