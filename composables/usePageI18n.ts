...
export function usePageI18n() {
  // useScope: 'global' so the writer (applyPageI18n) and the reader (LanguageToggle)
  ...
  /** Merge an page's messages without changing the active locale (for live preview). */
  function mergePageMessages(config: SiteConfig): void {
    for (const loc of config.locales) {
      // Escape transiently: page messages are user text that may contain i18n
  ...
  /**
   * Pick the initial locale for this page. Priority: the persisted cookie (read
   * server-side too) → the browser's language (Accept-Language on SSR,
   * navigator.language on client — they agree, so no flash) → the page default.
   * Pass `acceptLanguage` from the layout so SSR can detect without `navigator`.
   */
  function applyPageI18n(config: SiteConfig, acceptLanguage?: string): void {
    mergePageMessages(config)
  ...
  return { applyPageI18n, mergePageMessages, setLocale }
}
