...
export default defineEventHandler(async (event) => {
  ...
  if (!username || !userid) {
    if (org)
      void recordVerify(event, org.id, { outcome: 'missing', mode: null, name: null, idHash: null })
    return {
      ok: false,
      admitted: null,
  ...
  if (
    ...
    if (org) {
      void upsertVerifiedIdentity(org.id, normName, idHash)
  ...
  // --- Per-org "already used" fast-path: name + ID hash verified here before ---
  // Returning members are re-admitted without re-querying the UNNC portal.
  if (org && idHash && (await findVerifiedIdentity(org.id, idHash))) {
  ...
  // result so a cross-org skip can render the welcome page) + record the identity.
  if (result.ok && result.admitted === true) {
    const token = signVerifyJwt(normName, idHash ?? '', deviceHash, result)
    setVerifyCookie(event, token)
    if (org) void upsertVerifiedIdentity(org.id, normName, idHash)
  }
